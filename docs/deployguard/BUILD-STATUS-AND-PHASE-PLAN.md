...
| 1.4 | Generate a Tauri updater keypair, publish a static update manifest (same CDN/host as installers), set `pubkey`, flip `updater.active: true`; **test an N → N+1 update on a clean VM** | Release | 🟨 **Wired 2026-09-22** (v0.2.0): keypair generated (private key `~/.tauri/`, password in macOS Keychain), `pubkey` set, in-app check + "Update to x.y.z" offer (never auto-restarts), `latest.json` generated by the build script. Signature verified against the embedded pubkey with minisign; a one-byte tamper fails. **Still open:** create the public `creativesites/dogforce-desktop-releases` repo the endpoint points at, publish, and the N→N+1 test on a clean Windows VM |
...
| 1.6 | Structured logging to a rotating local file + a "copy diagnostics" button that produces the bundle a support ticket needs | Observability | ✅ 2026-09-22: daily-rotating files in the app log folder (7 kept, `info` default, blocking writes so a panic line survives `panic = "abort"`, panic hook), folder path in the existing copyable diagnostics panel. Rust test for the file writer. Also found: `from_default_env()` with no `RUST_LOG` had been dropping every `warn!` |
...
| 1.9 | First-run experience: monitoring notice + acknowledgement ([16](16-security-architecture.md) §9 Transparency — the "PR-IAM-11" tag previously here was wrong; PR-IAM-11 is sign-in rate limiting), and a plain-language "what this app is" screen | Product | ✅ 2026-09-22: welcome → notice → "Start my training". Acknowledgement recorded server-side (`security.deployguard.policy.acknowledgement`, versioned text, stale versions refused, own-records rule, admin audit list); Adoption hidden until acknowledged. Verified in a headless render of the real shell. **Notice text is a draft pending 5.1** |
| 1.10 | Session lifecycle honesty: detect Odoo session expiry and surface it (today an expired cookie likely reads as a generic error) | Desktop | 🟨 `AppError::SessionExpired` + `isExpired` in `SessionContext` landed in `849cb0e`; not yet exercised against a real expired session |
| 1.11 | Installer hygiene: per-user install, clean uninstall, no orphaned data, correct product/publisher metadata, versioning scheme decided (`0.x.y-pilot` → `1.0.0`) | Release | 🟨 Per-user NSIS install, publisher/copyright metadata, v0.2.0 across all three manifests. Clean uninstall untested (needs a Windows VM) |
...
| 4.8 | Automatic enrolment: courses reach new hires without a supervisor hand-assigning them | ERP | ✅ 2026-09-22: `auto_enroll` + optional role filter + due-within-days; enrols on publish and when switched on, hourly sweep for new hires/role changes. Only employees with their own internal login (adoption penalises overdue training, so enrolling someone who can't open the app would be unfair). Due dates from the learner's own timezone. 8 tests; 36/36 module tests on a fresh DB. **Off by default: a supervisor has to switch it on per course** |
...
| 2026-09-17 | Phase 6 (exceptions, inbox, notifications) built. New `security_exceptions` module ingests `security_notifications`' existing deterministic alerts 1:1 (no detection logic duplicated, per 6.2) into triaged, deduplicated exception instances with a two-level escalation ladder (wall-clock for Critical, `resource.calendar.get_work_hours_count`-based working-time for Attention/Watch), acknowledge-cancels-escalation, and pause-on-stale-data. Not built: the desktop manager/supervisor inbox UI (6.4, Odoo backend list view only for now), desktop OS notifications and preference/quiet-hours/digest delivery (6.5 — needs a Tauri build environment this session doesn't have), and the email provider decision (6.6 — a business decision, not code). |
| 2026-09-22 | Reconciled desktop claims against the code. Unlike the cowork board, the Phase 5–7 desktop rows held up (all 6 screens wired, 6.4 keyboard triage and 6.5 notifications verified end to end, 114/114 tests) — but ~5,700 lines of it were uncommitted; checkpointed in `1cdc64f`. V2 prep: training auto-enrolment (4.8), first-run onboarding + server-recorded monitoring notice (1.9), signed auto-updates + NSIS installer built from macOS (1.4), rotating log file (1.6). Found and fixed: 3 bridge tests that had never run and failed (a missing `security_operations` dependency, a compute without `@api.depends`, a wrong test); onboarding skipping its final step (caught by a headless render); 12 undefined CSS custom properties that left newer screens partly unstyled (cards rendering transparent) — lint 43 → 0. Rust tests 25 → 26. |
