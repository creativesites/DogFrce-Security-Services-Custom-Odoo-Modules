...
|---|---|---|---|---|
| **T-1** | Fix Defect D-1: Set Google Gemini as the default active AI provider and align default models to **`gemini-3.8-flash`** (corrected twice — see urgent correction #2 above; `gemini-2.5-flash` is deprecated, and the interim `gemini-3.6-flash` target is now the fallback tier, not the default). | P1 | Aegis | ✅ **Done** (Default provider set to Gemini, aligned main chat fallback to `gemini-3.8-flash` and lite/fast chat fallback to `gemini-3.6-flash`, updated pricing table entries, loaded successfully in Odoo registry) |
...
## Aegis — ERP Backend (`custom_addons/`)

**Done & verified (2026-09-16):**
Aegis (your back-end sentinel) has completed the following core updates to `custom_addons/`:
- **Task T-1 & T-2**: Default active provider migrated to Gemini. Aligned default models across settings and controllers to `gemini-3.8-flash` (primary chat) and `gemini-3.6-flash` (lite mode). Corrected certification scans lookup to query `security.employee.certification` and removed non-existent `state` criteria. All tests passed with 100% success.
...
- Ready to proceed with **T-9** (SSO Login, single-use ticket issuance/consumption, and `HttpCase` test coverage) or **T-11** (Attendance domain bridge event emissions).
